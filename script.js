...
// View More Products functionality with expand/collapse toggle
const viewMoreBtn = document.getElementById("view-more-btn")
const productGrid = document.querySelector(".product-grid")

if (viewMoreBtn && productGrid) {
  viewMoreBtn.addEventListener("click", () => {
    const isExpanded = productGrid.classList.toggle("expanded")
    viewMoreBtn.classList.toggle("expanded", isExpanded)
    
    const textSpan = viewMoreBtn.querySelector(".view-more-text span")
    if (textSpan) {
      textSpan.textContent = isExpanded ? "View Less" : "View More"
    }
  })
}
const headerOffset = navElement ? navElement.offsetHeight : 80