...
// View More Products functionality with expand/collapse toggle and scroll behavior
const viewMoreBtn = document.getElementById("view-more-btn")
const productGrid = document.querySelector(".product-grid")
const servicesSection = document.getElementById("services")
const navElement = document.querySelector("nav")

if (viewMoreBtn && productGrid && servicesSection) {
  viewMoreBtn.addEventListener("click", () => {
    const isExpanded = productGrid.classList.toggle("expanded")
    ...
    if (isExpanded) {
      // When expanding: scroll so navbar covers "Our Products" heading, first row of products visible
      setTimeout(() => {
        const sectionTop = servicesSection.getBoundingClientRect().top + window.scrollY
        const headingHeight = servicesSection.querySelector("h2").offsetHeight
        const scrollTarget = sectionTop + headingHeight + 10
        window.scrollTo({
          top: scrollTarget,
          behavior: "smooth"
        })
      ...
      // When collapsing: scroll back to show full section (like navbar Products link)
      setTimeout(() => {
